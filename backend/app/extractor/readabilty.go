...
import (
	...
	"github.com/ukeeper/ukeeper-redabilty/backend/app/datastore"
	readability "github.com/ukeeper/ukeeper-redabilty/backend/app/extractor/go-readabilty"
)
...
func (f UReadability) Extract(reqURL string) (rb *Response, err error) {
	...
	if err != nil {
		log.Printf("[WARN] failed to get anyting from %s, error=%v", reqURL, err)
		return nil, err
	}
...
func (f UReadability) normalizeLinks(data string, reqContext *http.Request) (result string, links []string) {
	...
	for _, m := range matches {
		...
		if absLink, changed := absoluteLink(srcLink); changed {
			...
			if f.Debug {
				log.Printf("[DEBUG] normlized %s -> %s", srcLink, dstLink)
			}
			normalizedCount++
		}
